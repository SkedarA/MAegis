...
export async function POST(request: Request, context: Context) {
  ...
  return proxy(request, context, "POST", { body: input.body.trim() });
}

async function proxy(request: Request, context: Context, method: "GET" | "POST", payload?: Record<string, unknown>) {
  ...
  try {
    const response = await fetch(url, {
      method,
      headers: backendHeaders(request, payload ? { "Content-Type": "application/json" } : {}),
