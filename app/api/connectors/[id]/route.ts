...
export async function PATCH(request: Request, context: Context) {
  ...
  const url = backendUrl(`/api/v1/connectors/${encodeURIComponent(id)}`);
  if (!url) return Response.json({ error: "Live API is not configured" }, { status: 409 });
  const input = await request.json() as { enabled?: boolean };
  if (typeof input.enabled !== "boolean") return Response.json({ error: "Invalid connector state" }, { status: 422 });
  try {
    ...
    return Response.json(response.ok ? { connector: body } : { error: body.detail ?? "Connector update failed" }, { status: response.status });
  } catch {
    return Response.json({ error: "Connector service unavailable" }, { status: 502 });
  }
}
